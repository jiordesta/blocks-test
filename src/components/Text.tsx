...
export default function Text({ block, modeEdit }: TextProps) {
  const { id, options, data } = block;
  const [showEditBlock, setShowEditBlock] = useState(false);
  const [showAddBlockData, setShowAddBlockData] = useState(false);
    ...
    return (
      <div className="fixed inset-0 w-full h-full flex justify-center items-center z-50 text-black">
        <div className="w-1/4 bg-slate-300 rounded-lg p-2 space-y-2">
          ...
          <button
            className={classBtn}
            onClick={() => setShowAddBlockData(false)}
          >
            Cancel
          </button>
        </div>
    ...
    return (
      <div className="top-0 p-1 z-40">
        <div className="flex gap-1">
          ...
          <button
            className="bg-black bg-opacity-5 p-1 rounded-full"
            onClick={() => setShowAddBlockData(true)}
          >
  ...
  return (
    <div className="relative w-full">
      ...
      {showAddBlockData && <AddText />}
      <h1 id={id} style={options}>
        {
          // @ts-ignore
          data ? data.value : " "
        }
